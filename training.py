...
from data import load_data, make_data_iter, load_data_test
...
class TrainManager:
    ...
    def save_skels(self, output_joints, display, model_dir, type, file_paths=None):
        ...
        for i in display:
            ...
            sign = torch.reshape(torch.tensor(seq, dtype = torch.float32),(seq.shape[0], 178, 3))
            example_dict[name] = sign

        torch.save(example_dict, model_dir+'/'+"predictions.pt") 
...
def test(cfg_file, ckpt: str):
    ...
    # load the data
    test_data, src_vocab, trg_vocab = load_data_test(cfg=cfg)
